...
class AccountTests(APITestCase):
    ...
    def test_post_without_image(self):
        url = self.base_url + 'snippets/'
        data = {'name': 'qwertyu2', 'about': '123456782', 'expiration_date': '2020-12-20', 'img_file': '',
                'status': 'Ready'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_post_400(self):
        url = self.base_url+'snippets/'
        data = {'about': '123456782','img_file': 'test.jpg', 'expiration_date': '2020-12-20', 'status': 'Ready'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_put_by_id_without_image(self):
        url = self.base_url+'snippets/1/'
        data = {'name': 'q', 'about': 'q', 'expiration_date': '2020-12-20', 'status': 'Ready', 'img_file': ''}
        response = self.client.put(url, data, format='json', kwargs={'pk': self.task.pk})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
